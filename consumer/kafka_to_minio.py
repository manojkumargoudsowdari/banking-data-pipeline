...
last_flush_at = {topic: time.time() for topic in buffer}


def flush_topic(topic):
    records = buffer.get(topic, [])
    if not records:
        return
...
try:
    for message in consumer:
        ...
        record = payload.get("after")  # Inserts/updates have "after"; deletes are ignored.
        ...
        if len(buffer[topic]) >= batch_size:
            flush_topic(topic)
        elif buffer[topic] and (now - last_flush_at[topic] >= flush_interval_seconds):
            flush_topic(topic)
except KeyboardInterrupt:
    print("Stopping consumer, flushing remaining buffered records...")
finally:
    flush_all()
    consumer.close()
